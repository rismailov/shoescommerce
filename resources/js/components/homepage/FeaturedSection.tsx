        ...
        <Link href={route('products.index')}>
            <div className="flex flex-col relative overflow-hidden rounded-2xl group">
                ...
                <div className="absolute inset-0 bg-gradient-to-tr from-accent-foreground/40 to-transparent p-8 flex flex-col justify-end">
                    <p className="text-xl text-accent/75">{preTitle}</p>

                    <h2 className="text-xl lg:text-2xl font-semibold text-accent">
                        {title}
                    </h2>

                    <Button className="self-start mt-3 bg-accent hover:bg-accent/90 text-accent-foreground rounded-full font-medium px-7 h-10">
                        {t('Shop')}
                    </Button>
                </div>
            </div>
